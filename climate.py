...
def get_climate(in_filename: str, out_filename: str) -> None:
    ...
    for line in in_file.readlines():
        ...
        # Converts string data into numbers.
        ...
        total_tempmin_year.setdefault(month_day, []).append(tempmin)
        total_tempmax_year.setdefault(month_day, []).append(tempmax)


    # Unsorted, but will be sorted as per assignment requirement.
    avg_precip = {month_day: mean(precip) for month_day, precip in total_precip.items()}
    avg_tempmin = {month_day: mean(tempmin) for month_day, tempmin in total_tempmin.items()}
    avg_tempmax = {month_day: mean(tempmax) for month_day, tempmax in total_tempmax.items()}
    ...
    record_tempmin_year = {month_day: min(tempmin) for month_day, tempmin in total_tempmin_year.items()}
    record_tempmax_year = {month_day: max(tempmax) for month_day, tempmax in total_tempmax_year.items()}
    ...
    for key in sorted_avg_precip.keys():
        out_handle.write("{},{},{},{},{},{},{},{}\n".format(key, str(sorted_avg_precip[key]), str(sorted_avg_tempmin[key]), str(sorted_avg_tempmax[key]),
        str(sorted_record_tempmin[key]), str(sorted_record_tempmax[key]), str(sorted_record_tempmin_year[key]), str(sorted_record_tempmax_year[key])))
